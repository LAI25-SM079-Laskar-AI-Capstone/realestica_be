...
@router.get("/facilities/unique")
async def get_unique_facilities(db: Session = Depends(get_db)):
    """Get all unique individual facilities across all properties"""
    
    try:
        # Get all facilities from all properties
        facilities_query = db.query(PropertyModel.facilities).filter(
            PropertyModel.facilities.isnot(None),
            PropertyModel.facilities != ""
        ).all()
        
        # Extract unique individual facilities
        unique_facilities = set()
        for facility_row in facilities_query:
            if facility_row[0]:  # Check if facilities is not None or empty
                # Split by semicolon and clean up each facility
                if isinstance(facility_row[0], str):
                    facilities_list = [f.strip() for f in facility_row[0].split(';') if f.strip()]
                    unique_facilities.update(facilities_list)
                elif isinstance(facility_row[0], list):
                    # If it's already a list, add all items
                    unique_facilities.update(facility_row[0])
        
        # Convert to sorted list
        facilities_list = sorted(list(unique_facilities))
        ...
        raise HTTPException(
            status_code=500,
            detail=create_error_response(500, "Failed to get unique facilities", {"exception": str(e)})
        )

@router.get("/poi-categories/unique")
async def get_unique_poi_categories(db: Session = Depends(get_db)):
    """Get all unique categories from nearby_points_of_interest_text across all properties"""
    
    try:
        # Get all POI texts from all properties
        poi_query = db.query(PropertyModel.nearby_points_of_interest_text).filter(
            PropertyModel.nearby_points_of_interest_text.isnot(None),
            PropertyModel.nearby_points_of_interest_text != ""
        ).all()
        
        # Extract unique categories
        unique_categories = set()
        
        for poi_row in poi_query:
            if poi_row[0]:  # Check if POI text is not None or empty
                poi_text = poi_row[0].strip()
                
                # Split by semicolon to get individual POI entries
                poi_entries = [entry.strip() for entry in poi_text.split(';') if entry.strip()]
                
                for entry in poi_entries:
                    # Look for pattern "Category: items"
                    if ':' in entry:
                        category = entry.split(':', 1)[0].strip()
                        if category:  # Only add non-empty categories
                            # Normalize category name (title case)
                            category_normalized = category.title()
                            unique_categories.add(category_normalized)
        
        # Convert to sorted list
        categories_list = sorted(list(unique_categories))
        
        return create_success_response({
            "poi_categories": categories_list,
        ...
        raise HTTPException(
            status_code=500,
            detail=create_error_response(500, "Failed to get unique POI categories", {"exception": str(e)})
        )